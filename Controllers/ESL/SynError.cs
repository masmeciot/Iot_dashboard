...
{
    public class SynError : Controller
    {
        public IActionResult Index()
        {
            return View("~/Views/ESL/Synerror.cshtml");
        }
        ...
        static string PerformLogin(string encryptedPassword)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(publicKeyText))
                {
                    ...
                    string json = "{\"account\": \"synerror123\",\"loginType\": 3,\"password\": \"" + encryptedPassword + "\"}";
        ...
        public IActionResult AddNewItem([FromBody] ItemData itemData)
        {
            try
            {
                string newPassword = "synerror@123";
