...
{
    public class machineInputs : Controller
    {
        private readonly AppDbContext1000 _dbContext;
        private readonly ILogger<machineInputs> _machineInputsLogger;

        public machineInputs(AppDbContext1000 dbContext, ILogger<machineInputs> machineInputsLogger)
        {
            _dbContext = dbContext;
            _machineInputsLogger = machineInputsLogger;
        }

        public IActionResult Index()
        {
            return View("~/Views/machineInputs.cshtml");
        }
        ...
        public async Task<IActionResult> GetUtilizeIotData(DateTime? date)
        {
            ...
            {
                _machineInputsLogger.LogError(ex, "An error occurred while fetching KreedaIOTTestNew data.");
                return StatusCode(500, "Internal server error");
            }
        }
    }

    public class AppDbContext10001 : DbContext
    {
        public DbSet<KreedaIOTTestNew> KreedaIOTTestNew { get; set; }

        public AppDbContext10001(DbContextOptions<AppDbContext10001> options) : base(options)
        {
        }
